...
// button
#define  BUTTON_RES_ACCEL  1
#define  BUTTON_SET_DECEL  2
#define  BUTTON_GAP_DIST   3
...
#define HYUNDAI_COMMUNITY_TX_MSGS(scc_bus) \
  {LKAS11,      0,        8, .check_relay = true},   /* LKAS11 Bus 0                              */ \
  {CLU11,       scc_bus,  4, .check_relay = false},  /* CLU11 Bus 0 (radar-SCC) or 2 (camera-SCC) */ \
  {LFAHDA_MFC,  0,        4, .check_relay = true},   /* LFAHDA_MFC Bus 0                          */ \
  {MDPS12,      2,        8, .check_relay = false},  /* MDPS12, Bus 2                             */ \


#define HYUNDAI_COMMUNITY_LONG_TX_MSGS(scc_bus) \
  HYUNDAI_COMMUNITY_TX_MSGS(scc_bus) \
  {SCC11,       0, 8, .check_relay = true}, /* SCC11 Bus 0       */ \
  {SCC12,       0, 8, .check_relay = true}, /* SCC12 Bus 0       */ \
  {SCC13,       0, 8, .check_relay = true}, /* SCC13 Bus 0       */ \
  {SCC14,       0, 8, .check_relay = true}, /* SCC14 Bus 0       */ \
  {0x4A2,       0, 2, .check_relay = false}, /* FRT_RADAR11 Bus 0 */ \


#define HYUNDAI_COMMUNITY_RX_CHECKS(legacy)                                                                                                                  \
  {.msg = {{EMS16, 0, 8, .max_counter = 3U, .frequency = 100U},                                                                                           \
           {E_EMS11, 0, 8, .ignore_checksum = true, .ignore_counter = true, .frequency = 100U}, { 0 }}},                                                    \
  {.msg = {{WHL_SPD11, 0, 8, .ignore_checksum = (legacy), .ignore_counter = (legacy), .max_counter = (legacy) ? 0U : 15U, .frequency = 100U}, { 0 }, { 0 }}}, \
  {.msg = {{TCS13, 0, 8, .ignore_checksum = (legacy), .ignore_counter = (legacy), .max_counter = (legacy) ? 0U : 7U, .frequency = 100U}, { 0 }, { 0 }}},  \
  {.msg = {{MDPS12, 0, 8, .ignore_checksum = true, .ignore_counter = true, .frequency = 50U}, { 0 }, { 0 }}},                                              \
  {.msg = {{CLU11, 0, 4, .ignore_checksum = true, .max_counter = 15U, .frequency = 50U}, { 0 }, { 0 }}},                                                  \

#define HYUNDAI_COMMUNITY_SCC12_ADDR_CHECK(scc_bus)                                     \
  {.msg = {{SCC12, (scc_bus), 8, .max_counter = 15U, .frequency = 50U}, { 0 }, { 0 }}}, \

#define HYUNDAI_COMMUNITY_FCEV_GAS_ADDR_CHECK \
  {.msg = {{0x91,  0, 8, .ignore_checksum = true, .ignore_counter = true, .frequency = 100U}, { 0 }, { 0 }}}, \

static const CanMsg _HYUNDAI_COMMUNITY_TX_MSGS[] = {
    HYUNDAI_COMMUNITY_TX_MSGS(0)
};
...
static bool hyundai_community_tx_hook( const CANPacket_t *to_send) {
  const TorqueSteeringLimits HYUNDAI_STEERING_LIMITS = HYUNDAI_COMMUNITY_LIMITS(384, 3, 7);
  const TorqueSteeringLimits HYUNDAI_STEERING_LIMITS_ALT = HYUNDAI_COMMUNITY_LIMITS(270, 2, 3);
  const TorqueSteeringLimits HYUNDAI_STEERING_LIMITS_ALT_2 = HYUNDAI_COMMUNITY_LIMITS(170, 2, 3);
  ...
  if ((addr == CLU11) && !hyundai_longitudinal) {
    ...
    bool allowed_resume = (button == BUTTON_RES_ACCEL) && controls_allowed; // acc
    bool allowed_set = (button == BUTTON_SET_DECEL) && controls_allowed;    // dec
    bool allowed_cancel = (button == BUTTON_CANCEL) && cruise_engaged_prev;
...
static safety_config hyundai_community_init(uint16_t param) {
  static const CanMsg HYUNDAI_LONG_TX_MSGS[] = {
    HYUNDAI_COMMUNITY_LONG_TX_MSGS(0)
  ...
  static const CanMsg HYUNDAI_CAMERA_SCC_TX_MSGS[] = {
    HYUNDAI_COMMUNITY_TX_MSGS(2)
  };

  static const CanMsg HYUNDAI_CAMERA_SCC_LONG_TX_MSGS[] = {
    HYUNDAI_COMMUNITY_LONG_TX_MSGS(2)
  };
  ...
  if (hyundai_longitudinal) {
    ...
    SET_TX_MSGS(_HYUNDAI_COMMUNITY_TX_MSGS, ret);
    if (hyundai_fcev_gas_signal) {
